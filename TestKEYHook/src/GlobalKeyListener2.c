    ...
    while (1) {
        if (isRecoilControlActive && (GetAsyncKeyState(VK_LBUTTON) & 0x8000)) {
            ...
            for (int i = 0; i < bulletCount; i++) { // 调整循环次数和移动量以适应不同的游戏和武器
                int randomX;
                // 检查是否在前recoilDelay毫秒内
                if (GetTickCount() - startTime < recoilDelay) {
                    randomX = 0; // 在前recoilDelay毫秒内随机值为0
                } else {
