...
#define down 下压
#define down_level 下压程度
#define down_for 下压次数

#define init_value 配置初始化
#define init_value_default 配置初始化的默认参数

#define key_in 键盘输入检测
#define key_in_F1 F1热键状态

#define function_run 具体功能
...
void 配置初始化() {
    配置初始化的默认参数.下压程度 = 3;
    配置初始化的默认参数.下压次数 = 6;
    配置初始化的默认参数.F1热键状态 = false;
}

void 键盘输入检测() {
    while (true) {
        if (GetAsyncKeyState(VK_F1) && (配置初始化的默认参数.F1热键状态 != true)) {
            cout << "正在开启功能" << endl;
            配置初始化的默认参数.F1热键状态 = true;
        }
        else if (GetAsyncKeyState(VK_F1) && (配置初始化的默认参数.F1热键状态 == true)) {
            cout << "正在关闭功能" << endl;
            配置初始化的默认参数.F1热键状态 = false;
        }
        Sleep(500);
...
void 具体功能() {
    while (true) {
        if (配置初始化的默认参数.F1热键状态 == true && GetAsyncKeyState(VK_RBUTTON)) {
            if (GetAsyncKeyState(VK_LBUTTON)) {
                for (int i = 0; i < 配置初始化的默认参数.下压次数; i++) {
                    mouse_event(MOUSEEVENTF_MOVE, 0, 配置初始化的默认参数.下压程度, 0, 0);
                    Sleep(40);
                }
            }
        }
...
int main() {
    cout << "欢迎使用TestKEYHook(C++轻量化版)" << endl;
    cout << "作者: Hirohiko360" << endl;
    cout << "使用F1键启动功能" << endl;

    配置初始化();

    thread keyInputThread(键盘输入检测);
    thread functionThread(具体功能);
